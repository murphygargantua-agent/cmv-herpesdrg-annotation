...
A database of CMV antiviral resistance mutations from HerpesDRG, mapped to AD169/NC_006273 coordinates, formatted for `vcfanno`.
...
2. **Deduplicate intervals** at the same start position — `build_database.py` now merges duplicates
...
5. **Chromosome names must match** — this DB uses `NC_006273`
...
Both databases use the same AD169 reference and can be combined by concatenating the BED files (after deduplication).
